...
This is a **Composer package** (`mrpunyapal/filament-infinite-select`), not an application. Document how a consumer installs it and the main use case. Do not write one page per source file.

A runnable demo lives in `playground/`. Boot it (`cd playground && php artisan serve`) only if the user asked for screenshots or videos, and capture from there, not from the package root.

This project is a **Filament plugin (v4)**. Panel path `/admin`.

- Capture screenshots or videos only if the user asked.

- Ask the user for demo credentials. Never invent emails or passwords.

- Login in `before` (off-camera): goto `/admin/login`, fill email/password, submit, wait for a `.fi-*` element. Do not record the login page unless login is the topic.

- Deep-link to the page that hosts the widget. `inspect`, then screenshot/video with `selector`. Form fields: `.fi-field`. Select overlays: `.fi-select-panel`.
